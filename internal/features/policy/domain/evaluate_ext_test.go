...
func TestEvaluateStrictestRuleWins(t *testing.T) {
	...
	got := domain.Evaluate(report, rules)
	if len(got) != 1 {
...
func TestEvaluateSkipsNotApplicable(t *testing.T) {
	...
	if got := domain.Evaluate(report, domain.DefaultRules()); len(got) != 0 {
...
func TestEvaluateToleratesFloatingPointNoiseAtBoundary(t *testing.T) {
	...
	rules := []domain.Rule{{Pattern: "**", Min: 0.5}}
...
func TestFormatViolations(t *testing.T) {
	...
	single := domain.FormatViolations([]domain.Violation{{Package: "p", Type: "T"}})
...
func TestValidate(t *testing.T) {
	...
	if err := domain.Validate(domain.DefaultRules()); err != nil {
	...
	cases := []struct {
		...
		{"empty pattern", []domain.Rule{{Pattern: "", Min: 0.5}}},
		{"nan min", []domain.Rule{{Pattern: "**", Min: math.NaN()}}},
		{"min below zero", []domain.Rule{{Pattern: "**", Min: -0.1}}},
		{"min above one", []domain.Rule{{Pattern: "**", Min: 1.1}}},
	}
	for _, tc := range cases {
		if err := domain.Validate(tc.rules); err == nil {
